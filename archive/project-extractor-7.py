...
class ProjectCodeExtractor:
    ...
    def collect_files(self):
        ...
        files_data.sort(key=lambda x: (len(x['path'].split('/')), x['path']))
        return files_data
        ...
        for file_data in files_data:
            ...
            if len(path_parts) > 1:
                ...
                if dir_name not in dirs:
                    dirs[dir_name] = []
                dirs[dir_name].append(Path(file_data['path']).name)
            else:
                if 'root' not in dirs:
                    dirs['root'] = []
                dirs['root'].append(file_data['path'])
        
        # Show top directories with their files
        for dir_name, files in list(dirs.items())[:5]:  # Show top 5 directories
            print(f"  📂 {dir_name}/")
            for file_name in files[:3]:  # Show max 3 files per directory
                print(f"     └─ {file_name}")
            if len(files) > 3:
    ...
    def extract(self):
        ...
        try:
            ...
            # Show ordered file list
            print(f"\n📋 Sample files:")
            for i, file_data in enumerate(files_data):
                if i < 25:  # Show first 25 files
                    print(f"   • {file_data['path']}")
                else:
                    print(f"   ... and {len(files_data) - 25} more files")
                    break
